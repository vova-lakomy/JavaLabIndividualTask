...
<div id="modal-upload-photo"
    ...
    <div id="upload-form"
        ...
        <ul>
                    ...
                    <c:choose>
                        <c:when test="${fullContact.photoLink eq null}">
                            <button class="jlab-button"
                                    disabled>${labels.get('download.image')}</button>
                        </c:when>
                        <c:otherwise>
                            <a href="${rootContext}contacts/${fullContact.photoLink}"
                               class="jlab-button"
                               download>${labels.get('download.image')}</a>
                        </c:otherwise>
                    </c:choose>
                </div>
            </li>
            ...
            <li class="jlab-row">
                <div class="jlab-cell-12">
                    <label class="jlab-button"
                           for="photo-file-input">${labels.get('upload.image')}</label>
        ...
        <div class="jlab-pull-right jlab-button-block">
            <button id="button-upload-photo"
                    type="button"
                    class="jlab-button">${labels.get('ok')}</button>
            <button id="button-cancel-upload-photo"
                    type="button"
